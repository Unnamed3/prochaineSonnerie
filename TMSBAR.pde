...
int mProchaineSonnerie = 0;
int hProchaineSonnerie = 0;
int a = 0;
int b = 0;
int c = 0;
int timeLeft=0;
...
void draw () {
  ...
  fill(#00E8FF);
  text("Tu n'est pas au lycée", screenX/2, screenY/2);
  text("Appuyer en dehors du boutton pour sortir", screenX/2, screenY*0.65);
  rectMode(CENTER);
  fill(255);
          if (highlight == true && highlightID == 1){
     fill(127);
   }
   else {fill (255);}
  rect(screenX/2, screenY*0.2, screenY*0.8, screenY*0.2);
  fill(0);
  text("Back to main screen",screenX/2, screenY*0.2);
  if (mousePressed){
  ...
  textAlign(CENTER, CENTER);
  text("Bienvenue sur TMSBAR", screenX/2, 0.07*screenY);
  ...
  rect(screenX/4, screenY/2, screenX*0.3, screenX*0.3);

     if (highlight == true && highlightID == 2){fill(127);}
   else {fill (255);}
  rect(screenX*0.75, screenY/2, screenX*0.3, screenX*0.3);
  textAlign(CENTER,CENTER);
  fill(0);
  text("Launch", screenX/4, screenY/2);
  text("Config", screenX*0.75, screenY/2);
 ...
 if (screen == 3){
    ...
    */
    ...
    rect(screenX*0.10,screenY*0.5, screenY*0.3, screenY*0.3);//rect -10
    if (highlight == true && highlightID == 2){
     fill(127);
   }
   else {fill (255);}
    rect(screenX*0.3,screenY*0.5, screenY*0.3, screenY*0.3);//rect -1
    if (highlight == true && highlightID == 3){
     fill(127);
   }
   else {fill (255);}
    rect(screenX*0.7,screenY*0.5, screenY*0.3, screenY*0.3);//rect +1
    if (highlight == true && highlightID == 4){
     fill(127);
   }
   else {fill (255);}
    rect(screenX*0.9,screenY*0.5, screenY*0.3, screenY*0.3);//rect +1
    
    fill(255);
        if (highlight == true && highlightID == 5){
     fill(127);
   }
   else {fill (255);}
    rect(screenX*0.35, screenY*0.82, screenY*0.5, screenY*0.2);
        if (highlight == true && highlightID == 6){
     fill(127);
   }
   else {fill (255);}
    rect(screenX*0.65, screenY*0.82, screenY*0.5, screenY*0.2);
    fill(0);
    textAlign(CENTER, CENTER);
    text("Back", screenX*0.35, screenY*0.82);
    text("Save", screenX*0.65, screenY*0.82);
    text("-10",screenX*0.10,screenY*0.5);
    
    text("-1",screenX*0.3,screenY*0.5);
    text(SStemp, screenX/2, screenY/2);
    
    text("+1",screenX*0.7,screenY*0.5);
    text("+10",screenX*0.9,screenY*0.5);
 ...
 if (screen == 2){
  ...
  textAlign(CENTER, CENTER);
  text("Progression du cours : " + pourcentCours+"%", 0.5*screenX,0.7*screenY);
